...
export function PriceChart({ symbol, exchange = 'NSE', livePrice, mainLiveLoading }: PriceChartProps) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      rightPriceScale: { borderColor: 'hsl(220, 30%, 20%)' },
      timeScale: {
    ...
    const hasLiveGraph = liveGraph && liveGraph.length > 0;
    ...
    if (hasLiveGraph) {
      ...
      if (volumePoints.length > 0) {
        ...
        const volData = volumePoints.map((point, i) => {
          ...
          return { time: time as UTCTimestamp, value: point.volume!, color: 'hsla(187, 100%, 50%, 0.2)' };
        });
      ...
      candleSeries.setData(candleData.map(d => ({ ...d, time: d.time as UTCTimestamp })));
      ...
      if (showSMA) {
        ...
        if (sma20Data.length > 0) {
          chart.addSeries(LineSeries, {
            color: 'hsl(187, 100%, 50%)', lineWidth: 1, priceLineVisible: false, lastValueVisible: false,
          }).setData(sma20Data);
        }
        if (sma50Data.length > 0) {
          chart.addSeries(LineSeries, {
            color: 'hsl(45, 100%, 55%)', lineWidth: 1, priceLineVisible: false, lastValueVisible: false,
          }).setData(sma50Data);
        }
      }
    }
      ...
      <CardHeader className="pb-2">
        ...
        <div className="flex gap-4 text-xs text-muted-foreground mt-1">
          <span className="flex items-center gap-1">
            ...
            {liveGraph && liveGraph.length > 0 ? `Live ${timeframe}` : 'SMA 20'}
          </span>
          {!(liveGraph && liveGraph.length > 0) && (
            <span className="flex items-center gap-1">
